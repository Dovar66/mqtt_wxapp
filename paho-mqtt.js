...
(function ExportLibrary(root, factory) {
  ...
  var PahoMQTT = (function(wx) {
    ...
    ClientImpl.prototype.send = function(message, onSuccess, onFailure) {
      this._trace("Client.send", message);

      var wireMessage = new WireMessage(MESSAGE_TYPE.PUBLISH);
      wireMessage.payloadMessage = message;
      if (typeof onSuccess === 'function') {
        wireMessage.onSuccess = onSuccess
      }
      if (typeof onFailure === 'function') {
        wireMessage.onFailure = onFailure
      }
    ...
    ClientImpl.prototype._socket_send = function(wireMessage) {
      ...
      wx.sendSocketMessage({
        ...
        success: function() {
          if (wireMessage.type == MESSAGE_TYPE.CONNECT || //建立长连的消息要在收到服务器的MESSAGE_TYPE.CONNACK之后才回调onSuccess
            wireMessage.type == MESSAGE_TYPE.SUBSCRIBE || //订阅和取消订阅的消息要在收到服务器响应SUBACK/UNSUBACK之后才回调
            wireMessage.type == MESSAGE_TYPE.UNSUBSCRIBE
          ) {
            return
          }
          if (wireMessage.onSuccess) {
            wireMessage.onSuccess()
          }
        },
        fail: function() {
          if (wireMessage.type == MESSAGE_TYPE.CONNECT || //建立长连的消息要在client._disconnected()之后才回调onFailure
            wireMessage.type == MESSAGE_TYPE.SUBSCRIBE || //订阅和取消订阅的消息要在收到服务器响应SUBACK/UNSUBACK之后才回调
            wireMessage.type == MESSAGE_TYPE.UNSUBSCRIBE) {
            return
          }
          if (wireMessage.onFailure) {
            wireMessage.onFailure()
          }
        },
        complete: function() {
          // 
    ...
    var Client = function(host, port, path, clientId) {
       ...
       */
      this.send = function(topic, payload, qos, retained, onSuccess, onFailure) {
        var message;
        ...
        if (arguments.length === 0) {
          ...
          client.send(message, onSuccess, onFailure);
        }
      };
       ...
       * @throws {InvalidState} if the client is not connected.
       */
      this.publish = function(topic, payload, qos, retained, onSuccess, onFailure) {
